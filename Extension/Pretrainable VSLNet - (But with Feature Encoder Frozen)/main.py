...
def main(configs, parser):
    ...
    if configs.mode.lower() == "train":
        ...
        # **MANUALLY LOAD PRETRAINED Egovlp WEIGHTS** if provided
        if getattr(configs, "pretrained_ckpt", None):
            print(f"⏳ Loading pretrained weights from {configs.pretrained_ckpt}")
            ckpt = torch.load(configs.pretrained_ckpt, map_location="cpu")
            # some .t7 files store a bare state_dict, some wrap it under 'model_state_dict'
            state_dict = ckpt.get("model_state_dict", ckpt)
            model.load_state_dict(state_dict, strict=False)
            print("✅ Backbone initialized from pretrained checkpoint")
        
        
                # 1) Freeze “backbone” (video proj, text proj, feature encoder)
        for name, module in model.named_children():
            if name in ["video_affine", "embedding_net", "feature_encoder"]:
                for p in module.parameters():
                    p.requires_grad = False

        # 2) Unfreeze only the last-stage reasoning heads
        for name, module in model.named_children():
            if name in [
                "cq_attention",    # context–query attention
                "cq_concat",       # fusion
                "highlight_layer", # query-guided highlight
                "predictor",       # span start/end predictor
                "query_affine"     # (only exists when using BERT)
            ]:
                for p in module.parameters():
        ...
        for n, p in model.named_parameters():
            print(f"{n:50s} | requires_grad={p.requires_grad}") 
